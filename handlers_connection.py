...
async def connect_spot_netapp_connector(ctx, params: ConnectParams) -> ActionResult:
...
async def disconnect_spot_netapp_connector(ctx, params: ConnectionIdParams) -> ActionResult:
    conns = await _load_conns(ctx)
    if not conns:
