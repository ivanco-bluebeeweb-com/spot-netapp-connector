"""Connection lifecycle for Spot by NetApp Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from spot_netapp_client import SpotByNetappClient
...
_SECRET = "spot_netapp_connections"
...
async def connect_spot_netapp(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Spot by NetApp Connector."""
    ...
    record = {
        "id": cid,
        "label": params.label or "Spot by NetApp Account",
...
@chat.function(
    "list_connections",
    "List connected Spot by NetApp accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
async def disconnect_spot_netapp(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
