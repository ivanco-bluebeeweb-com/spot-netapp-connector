...
async def connect_spot_netapp_connector(params: ConnectParams, ctx) -> ActionResult:
    ...
    return ActionResult.ok(rec, summary=f"Connected Spot by NetApp ({rec['label']}).")
...
async def disconnect_spot_netapp_connector(params: ConnectionIdParams, ctx) -> ActionResult:
    ...
    if params.connection_id:
        ...
        conns.clear()
    await _save_conns(ctx, conns)
    return ActionResult.ok({"success": True, "message": "Disconnected successfully."}, summary="Disconnected Spot by NetApp connection.")
